...
The latest sanitized aggregate baseline is kept in the GitHub Actions cache at `.partner-metrics/cloud-baseline.json`; it is never committed. Raw API responses, customer keys and customer email addresses are never written or logged. Every successful run sends a consolidated aggregate dashboard through Resend's verified system sender to `hello@artificial.one`; the receiving address remains a forwarding-only inbox and no Artificial.One mailbox credentials are used. A first run initializes the baseline. Later runs also create a generic GitHub issue assigned to the repository owner only when a meaningful aggregate change is detected. The public workflow summary and issue deliberately contain no revenue or customer details.
