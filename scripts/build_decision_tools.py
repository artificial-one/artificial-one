...
def render_calculator(spec: dict[str, Any], offers: list[dict[str, Any]]) -> str:
    ...
    script = f'''<script>(function(){{function num(id){{var el=document.getElementById(id),n=Number(el.value);return Number.isFinite(n)?Math.max(Number(el.min)||0,Math.min(Number(el.max)||1e12,n)):0;}}function money(n){{return new Intl.NumberFormat('en-US',{{style:'currency',currency:'USD',maximumFractionDigits:0}}).format(n);}}function update(){{var v={{{values}}};var r=(function(v){{{spec['formula']}}})(v);document.getElementById('result-value').textContent=r.primary;document.getElementById('result-label').textContent=r.label;document.getElementById('result-detail').textContent=r.detail;}}document.querySelectorAll('.calc-input').forEach(function(x){{x.addEventListener('input',update);}});document.getElementById('share-result').addEventListener('click',async function(){{var url=new URL(location.href);url.searchParams.set('utm_source','shared-calculator');url.searchParams.set('utm_medium','referral');url.searchParams.set('utm_campaign','{spec['slug']}');var data={{title:'{esc(spec['title'])}',text:document.getElementById('result-label').textContent+': '+document.getElementById('result-value').textContent,url:url.toString()}};try{{if(navigator.share){{await navigator.share(data);}}else{{await navigator.clipboard.writeText(data.text+' '+data.url);document.getElementById('share-status').textContent='Share link copied.';}}}}catch(_e){{}}}});update();}})();</script>'''
    content = f'''<section class="bg-white"><div class="mx-auto max-w-5xl px-5 py-16"><p class="text-sm font-bold uppercase tracking-widest text-indigo-600">Free calculator</p><h1 class="mt-4 text-4xl font-black md:text-6xl">{esc(spec['title'])}</h1><p class="mt-5 max-w-3xl text-lg text-slate-600">{esc(spec['description'])}</p><div class="mt-9 grid gap-5 rounded-3xl border border-indigo-100 bg-indigo-50 p-6 md:grid-cols-2">{fields}</div><div class="mt-6 rounded-3xl bg-slate-900 p-7 text-white"><p id="result-label" class="text-sm font-bold uppercase tracking-widest text-indigo-300"></p><p id="result-value" class="mt-2 text-5xl font-black"></p><p id="result-detail" class="mt-3 text-slate-300"></p><button id="share-result" type="button" class="mt-5 rounded-lg border border-slate-600 px-4 py-2 font-bold text-white">Share this calculator</button><span id="share-status" class="ml-3 text-sm text-emerald-300" aria-live="polite"></span></div><p class="mt-4 text-xs text-slate-500">Illustrative estimate only. Results depend entirely on your inputs and are not a performance promise.</p></div></section><section class="mx-auto max-w-5xl px-5 py-10"><h2 class="text-3xl font-black">Tools relevant to this calculation</h2><div class="mt-6 grid gap-5 md:grid-cols-2">{cards}</div></section>{script}'''
    return shell(
