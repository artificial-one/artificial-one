...
def render_recipe_hub(payload: dict[str, Any]) -> str:
    cards = "".join(f'''<a class="recipe-card" href="workflow-recipes/{esc(item['id'])}.html" data-recipe-search="{esc((item['title'] + ' ' + item['outcome']).casefold())}"><span class="elephant-mini" aria-hidden="true"></span><h2>{esc(item['title'])}</h2><p>{esc(item['outcome'])}</p><strong>Open the workflow →</strong></a>''' for item in payload["recipes"])
...
def render_observatory(payload: dict[str, Any]) -> str:
    def buyer_message(item: dict[str, Any]) -> str:
        kind = str(item.get("kind") or "product").casefold()
        if "pricing" in kind or "plan" in kind:
            return "The vendor appears to have changed its pricing or plan details. Check the current offer before you subscribe or renew."
        if "availability" in kind:
            return "Availability appears to have changed. Confirm that the product or plan works in your region before you rely on it."
        return "The product appears to have changed. Review the current capabilities before you choose or renew it."

    changes = payload["changes"]
    latest = "".join(f'''<article class="change-card" data-change-kind="{esc(item.get('kind', 'product'))}"><p class="eyebrow">{esc(item.get('kind', 'product'))} · {esc(item.get('detected_on', ''))}</p><h3>{esc(item.get('tool_name', 'AI tool'))}</h3><p>{esc(buyer_message(item))}</p><div class="button-row"><a class="link-subtle" href="ai-tool-database.html?q={esc(item.get('tool_id', ''))}">See tool details</a><a class="link-subtle" href="{esc(item.get('source_url', ''))}" target="_blank" rel="noopener">Check vendor page ↗</a><button class="btn btn-small" type="button" data-observatory-watch="{esc(item.get('tool_id', ''))}" data-tool-name="{esc(item.get('tool_name', 'AI tool'))}">Watch this tool</button></div></article>''' for item in changes[:30]) or '<div class="surface"><h2>You are up to date</h2><p>No important price, plan, availability or product change needs your attention right now.</p></div>'
    stats = payload["stats"]
    content = f'''<section class="observatory-hero"><div class="container"><p class="eyebrow">Buy with current information</p><h1>AI tool price &amp; product changes</h1><p class="hero-copy">See which tools changed their price, plan or availability before you buy. We verify a change more than once, then show you the source and let you follow the tools you care about.</p><div class="stat-grid"><div><strong>{stats['changes']}</strong><span>recent changes worth checking</span></div><div><strong>{stats['tools']}</strong><span>tools checked for updates</span></div><div><strong>Free</strong><span>personal watchlist</span></div><div><strong>Email</strong><span>optional change alerts</span></div></div></div></section><section class="container section"><div class="answer-head"><div><p class="eyebrow">What changed</p><h2>Check before you subscribe</h2></div><label>Filter<select data-change-filter><option value="all">All changes</option><option value="pricing or plan">Pricing or plan</option><option value="availability">Availability</option><option value="product">Product</option></select></label></div><div class="change-grid" data-change-grid>{latest}</div><section class="watch-center"><div><p class="eyebrow">No account needed</p><h2>Follow the tools you care about</h2><p>Choose “Watch tool” below and optionally add your email. We will alert you only when a useful price, plan, availability or product change is confirmed.</p><div data-observatory-watchlist class="watch-pills"></div></div><form data-watch-email-form><label>Email for useful tool changes<input name="email" type="email" maxlength="180" required placeholder="you@example.com"></label><input name="website" tabindex="-1" autocomplete="off" class="honey"><label class="consent"><input name="consent" type="checkbox" required> Email me only about confirmed changes to tools I follow. I can unsubscribe at any time.</label><button class="btn" type="submit">Turn on email alerts</button><p data-watch-status aria-live="polite"></p></form></section></section>{script_tags()}'''
    return shell(title="AI Tool Price & Product Changes | artificial.one", description="Check recent AI software price, plan, availability and product changes before you subscribe.", canonical_path="ai-tool-observatory.html", content=content, structured_data={"@context": "https://schema.org", "@type": "Dataset", "name": "Artificial.One AI Tool Price and Product Changes", "dateModified": payload["updated_at"], "distribution": [{"@type": "DataDownload", "encodingFormat": "application/json", "contentUrl": "https://artificial.one/data/elephant_experience.json"}]})
...
def render_confirmed() -> str:
    content = '''<section class="elephant-hero"><div class="container"><div class="elephant-orb" aria-hidden="true"></div><p class="eyebrow">Watchlist confirmed</p><h1>The elephant is listening</h1><p class="hero-copy">Your selected tools are now watched. You will receive an email only when a useful price, plan, availability or product change is confirmed.</p><a class="btn" href="ai-tool-observatory.html">See tool changes</a></div></section>'''
