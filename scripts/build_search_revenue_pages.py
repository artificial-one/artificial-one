...
def render_value_calculator(offer: dict[str, Any]) -> str:
    use_cases = "".join(f"<li>{esc(item)}</li>" for item in offer.get("use_cases", []))
    calculator = f'''<div class="rounded-2xl border border-indigo-200 bg-indigo-50 p-6"><div class="grid gap-5 md:grid-cols-3"><label class="font-bold">Estimated monthly price<input id="tool-cost" class="mt-2 w-full rounded-lg border border-slate-300 bg-white p-3" type="number" min="0" step="1" value="50"></label><label class="font-bold">Hours saved monthly<input id="hours-saved" class="mt-2 w-full rounded-lg border border-slate-300 bg-white p-3" type="number" min="0" step="1" value="10"></label><label class="font-bold">Value of one hour<input id="hour-value" class="mt-2 w-full rounded-lg border border-slate-300 bg-white p-3" type="number" min="0" step="1" value="35"></label></div><div class="mt-6 grid gap-4 md:grid-cols-3"><p class="rounded-xl bg-white p-4"><span class="block text-sm text-slate-500">Monthly value</span><strong id="monthly-value" class="text-2xl">$350</strong></p><p class="rounded-xl bg-white p-4"><span class="block text-sm text-slate-500">Net monthly value</span><strong id="net-value" class="text-2xl">$300</strong></p><p class="rounded-xl bg-white p-4"><span class="block text-sm text-slate-500">Estimated ROI</span><strong id="roi-value" class="text-2xl">600%</strong></p></div><button id="share-value-result" type="button" class="mt-5 rounded-lg bg-indigo-600 px-4 py-2 font-bold text-white">Share this calculator</button><span id="share-value-status" class="ml-3 text-sm text-indigo-800" aria-live="polite"></span></div><script>(function(){{var cost=document.getElementById('tool-cost'),hours=document.getElementById('hours-saved'),value=document.getElementById('hour-value');function money(number){{return new Intl.NumberFormat('en-US',{{style:'currency',currency:'USD',maximumFractionDigits:0}}).format(number)}}function update(){{var c=Math.max(0,Number(cost.value)||0),h=Math.max(0,Number(hours.value)||0),v=Math.max(0,Number(value.value)||0),gross=h*v,net=gross-c,roi=c?net/c*100:0;document.getElementById('monthly-value').textContent=money(gross);document.getElementById('net-value').textContent=money(net);document.getElementById('roi-value').textContent=(c?roi.toFixed(0):'0')+'%'}}[cost,hours,value].forEach(function(input){{input.addEventListener('input',update)}});document.getElementById('share-value-result').addEventListener('click',async function(){{var url=new URL(location.href);url.searchParams.set('utm_source','shared-calculator');url.searchParams.set('utm_medium','referral');url.searchParams.set('utm_campaign','{esc(offer['slug'])}-value');var data={{title:'{esc(offer['name'])} value calculator',text:'Estimate the value of {esc(offer['name'])} for your workflow.',url:url.toString()}};try{{if(navigator.share){{await navigator.share(data)}}else{{await navigator.clipboard.writeText(data.text+' '+data.url);document.getElementById('share-value-status').textContent='Share link copied.'}}}}catch(_e){{}}}});update()}})();</script>'''
